...
The Operating Cash Flow lesson uses a separate statement contract rather than
presenting cash-flow facts as income-statement lines. The endpoint returns the
FY2025 operating reconciliation from Net Income through `Cash generated by
operating activities` in one coherent annual filing context.
...
2. Require every reconciliation line to match its accession, start date, end
   date, fiscal year, form, and USD unit.
3. Collapse only identical duplicates and reject missing, conflicting, or
   cross-filing facts.
4. Return signed statement-display values, stable FinPath ids, SEC taxonomy
   tags and labels, roles, filing-index provenance, and retrieval status.
5. Keep Apple's exact presentation labels in accession-bound reviewed teaching
