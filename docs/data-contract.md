...
## FY cash-flow-statement endpoint

```http
GET /v1/companies/{ticker}/cash-flow-statements/{fiscal_year}
```

The Cash Flow lessons use a separate statement contract rather than presenting
cash-flow facts as income-statement lines. The endpoint returns three ordered
sections—Operating, Investing, and Financing—plus the reported net change and
beginning/ending cash balances in one coherent annual filing context.

The response model is reusable, while taxonomy-row and sign rules are selected
from an explicitly reviewed company/fiscal-year statement profile. V0 contains
only the Apple FY2025 profile. An unreviewed company fails as unsupported
instead of silently inheriting Apple's row requirements.

The SEC Company Facts taxonomy expresses some asset and liability changes as
economic increases/decreases, while Apple's cash-flow statement displays their
signed effect on operating cash. The extractor applies explicit, tag-specific
sign normalization and then requires the complete ordered reconciliation to sum
exactly to the reported operating-cash total.

Selection and trust boundaries match the income-statement contract:

1. Select the requested annual operating-cash total as the statement anchor.
2. Require every duration fact to match its accession, start date, end
   date, fiscal year, form, and USD unit.
3. Require the beginning and ending cash facts to match the same filing,
   fiscal year, form, filed date, and exact boundary dates.
4. Collapse only identical duplicates and reject missing, conflicting, or
   cross-filing facts.
5. Validate the Operating, Investing, and Financing section totals, then
   require those totals to reconcile to the reported net change and require
   beginning cash plus that change to equal ending cash.
6. Return signed statement-display values, stable FinPath ids, SEC taxonomy
   tags and labels, roles, filing-index provenance, and retrieval status.
7. Keep Apple's exact presentation labels in accession-bound reviewed teaching
   content; Company Facts taxonomy labels do not substitute for filed labels.

Free Cash Flow is not returned as an Apple-reported statement line. The web
lesson derives its selected simple convention as Operating Cash Flow minus the
absolute value of purchases of property, plant and equipment, after validating
both inputs against this statement contract. The evidence layer identifies the
inputs as reported and the result as FinPath-derived, and states that Free Cash
Flow is non-GAAP and definitions can vary.
