...
test("classifies Operating Cash Flow as reported and only transforms its display unit", () => {
  ...
  const reviewedPresentation = buildReviewedPresentation({
    statement,
    content: {
      fiscalYear: 2025,
      startDate: "2024-09-29",
    ...
    contextLineIds: ["cash-generated-by-operating-activities"],
  });
  const evidence = buildReportedEvidence({
  ...
  assert.equal(evidence.metric.id, "operating-cash-flow");
  assert.equal(evidence.finPathDisplay.value, 111.482);
  assert.equal(evidence.transformation.inputValue, 111_482);
  assert.equal(evidence.transformation.divisor, 1_000);
  assert.equal(evidence.transformation.note, "Formatting only. No financial estimate.");
});
...
test("the reported inspector teaches the reviewed million-to-billion conversion first", async () => {
  ...
  assert.match(component, /value < 0 \? `\(\$\{formatted\}\)` : formatted/);
