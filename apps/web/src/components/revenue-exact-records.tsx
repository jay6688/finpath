...
export function RevenueExactRecords({ overview }: { overview: CompanyOverview }) {
  ...
  return (
    <section className="exact-record" aria-labelledby="exact-record-heading">
      <div className="exact-record__heading">
        ...
        <span>Values in {overview.metric.currency}</span>
      </div>

      <table className="revenue-table">
        <caption className="sr-only">
          Annual Revenue values in {overview.metric.currency} and source filings
        </caption>
        <thead>
          <tr>
      ...
      <div
        ...
        aria-label={`Annual Revenue values in ${overview.metric.currency}`}
      >
        {orderedSeries.toReversed().map((fact, index) => (
          ...
          >
            <summary>
              <span><strong>FY{fact.fiscalYear}</strong><small>Revenue</small></span>
              <b>{compactCurrency.format(fact.value)}</b>
            </summary>
            <dl>
              <div><dt>Exact value</dt><dd>{exactCurrency.format(fact.value)}</dd></div>
              <div><dt>Period ended</dt><dd>{fact.endDate}</dd></div>
              <div><dt>Filed</dt><dd>{fact.filedAt}</dd></div>
