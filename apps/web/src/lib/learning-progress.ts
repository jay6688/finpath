...
const recommendations: Record<ConceptId, Omit<HomeRecommendation, "conceptId" | "action">> = {
  ...
  "operating-cash-flow": {
    ...
    href: "/company/aapl/operating-cash-flow",
  },
};
...
export function deriveHomeRecommendation(
  ...
  if (!current) {
    return {
      ...
      ...recommendations["operating-cash-flow"],
      action: "Review",
    };
  }
...
export function deriveUpNextModel(
  ...
  if (currentConceptId === "operating-cash-flow") {
    return {
      kind: "coming-later",
      eyebrow: "More concepts coming",
  ...
  const next = currentConceptId === "revenue-growth"
    ? {
          title: "Profit / Net Income",
          reason:
    ...
    : currentConceptId === "profit"
      ...
      : {
          title: "Operating Cash Flow",
          reason:
            "Profit is an accounting result. Next, see how Net Income connects to cash generated by the business.",
          href: "/company/aapl/operating-cash-flow",
        };
