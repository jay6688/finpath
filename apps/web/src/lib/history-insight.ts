...
export function buildYearOverYearObservations(
  ...
  return buildRevenueGrowthRows(series)
    .filter((row): row is Extract<RevenueGrowthRow, { state: "available" }> =>
      row.state === "available"
    )
    .map(({ previous, current, absoluteChange, percentageChange, direction }) => ({
      previous,
      current,
      absoluteChange,
      percentageChange,
